...
###############
#
# The MasOS-Manager's manager.
#
# This script MUST be in the same directory MasOS-Manager's directory.
# If it's installed in "/opt/masos/supplementary/retropie-manager/",
# and only in this case, the user can make symbolic links to it. Otherwise,
# the symbolic links won't work.
#
# The MasOS-Manager can not be started directly by the root user, but
# but if you need it (ex.: on boot), use the --user option. The user must
# be a MasOS user (must have a MasOS directory tree in its homedir).
...
# "/opt/masos/supplementary/retropie-manager/logs" ($rpmanager_dir/logs)
...
--start             start the MasOS-Manager

--stop              stop the MasOS-Manager

--isrunning         show if MasOS-Manager is running and the
                    listening port and exit

--log               save the log messages (optional, default: not save log
                    messages, only works with --start)

-u|--user USER      start MasOS-Manager as USER (only available for
...
##############################################################################
# Set the user that will start the MasOS-Manager. Only privileged users
# can use this function. The user MUST have a MasOS directory tree in
# its homedir.
#
# Globals:
#   user
# Arguments:
#   $1  a valid MasOS user name
# Returns:
...
function set_user() {
    ...
    if [[ ! -d "/home/$1/MasOS/" ]]; then
        echo "Error: the user '$1' is not a MasOS user." >&2
        return 1
    fi
...
##############################################################################
# Check if MasOS-Manager is running. If positive, fill the $port global
...
##############################################################################
# Starts the MasOS-Manager service. It gives an error if it's called
# directly by the root user, but if it's called by an "sudo environment",
# MasOS-Manager is started by the sudo user.
#
# Globals:
#   log_command
#   user
...
#   None
# Returns:
#   1, if it's unable to start MasOS-Manager
#   0, if MasOS-Manager is successfully started
##############################################################################
function start_service() {
    if is_running; then
        echo "Nothing done. MasOS-Manager is already running and listening at $port." >&2
        return 1
    fi
    ...
    # MasOS-Manager should not be started directly by the root user,
    # but we can deal if it's called by a "sudo environment" or with
    # the --user option.
    if [[ $(id -u) -eq 0 ]]; then
        if [[ $(id -u "$user") -eq 0 ]]; then
            echo "Error: MasOS-Manager can't be started directly by root!" >&2
    ...
    echo "Starting MasOS-Manager..."
    eval $startcmd &>/dev/null &
    sleep 3
    if is_running; then
        echo "MasOS-Manager is running and listening at port $port"
        return 0
    else
        echo "Error: It seems that MasOS-Manager had some problem to start!" >&2
        return 1
    fi
}
...
##############################################################################
# Stops the MasOS-Manager service if it's running.
#
# Globals:
#   None
# Arguments:
#   None
# Returns:
#   0, if MasOS-Manager process is successfully killed
#   1, if unable to kill MasOS-Manager
#   2, if MasOS-Manager wasn't running from the start
##############################################################################
function stop_service() {
    if is_running; then
        echo "Stopping MasOS-Manager..."
        sudo kill -9 $(pgrep -f 'python.*manage\.py.*runserver')
        sleep 1
        if is_running; then
            echo "Error: Unable to kill MasOS-Manager process." >&2
            return 1
        else
            echo "MasOS-Manager has been stopped."
            return 0
        fi
    fi

    echo "Nothing done. MasOS-Manager wasn't running." >&2
    return 2
}
...
if [ ! -x "$rpmanager_dir/bin/python" -a ! -f "$rpmanager_dir/manage.py" ]; then
    rpmanager_dir="/opt/masos/supplementary/retropie-manager"
    if [[ ! -d "$rpmanager_dir" ]]; then
        echo "Error: $(basename $0) MUST be in the MasOS-Manager's directory" >&2
        exit 1
    fi
fi
...
while [[ "$1" ]]; do
    ...
    --isrunning)
        if is_running; then
            ...
            echo "MasOS-Manager is not running"
            exit 1
        fi
    ;;
